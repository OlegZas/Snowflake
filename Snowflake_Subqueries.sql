...
WITH RANKS AS (
    SELECT DENSE_RANK() OVER(ORDER BY SALARY DESC) AS RANKS2, SALARY, EMPLOYEE_ID
    FROM EMPLOYEES E )
SELECT RANKS2, E.SALARY
FROM EMPLOYEES E
JOIN RANKS ON E.SALARY = RANKS.SALARY
WHERE RANKS2 = 5
GROUP BY RANKS2, E.SALARY

-- 1. FIND DUPLICATES: 
select EMPLOYEE_ID, COUNT(1) FROM EMPLOYEES GROUP BY EMPLOYEE_ID HAVING COUNT(EMPLOYEE_ID)>1 ;
...
WHERE EMPLOYEE_ID IN (
    ...
    FROM (
        SELECT 
            EMPLOYEE_ID,
            ROW_NUMBER() OVER (PARTITION BY EMPLOYEE_ID ORDER BY EMPLOYEE_ID ASC) AS RANKS2
        FROM EMPLOYEES
        WHERE EMPLOYEE_ID IN (
            SELECT EMPLOYEE_ID
            FROM EMPLOYEES
    ...
    WHERE RANKS.RANKS2 > 1
);
-- 1/22/25
/* -- 17. Write a MySQL query to find the 4th minimum salary in the employees' table. */
WITH RANKED AS (
    SELECT  ROW_NUMBER() OVER(ORDER BY SALARY ASC) AS SALARY_ORD,SALARY, EMPLOYEE_ID 
                    FROM EMPLOYEES 
